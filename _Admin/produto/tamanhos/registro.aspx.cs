namespace loja
{
    ...
    public partial class _Admin_produto_tamanhos_registro : System.Web.UI.Page
    {
        ...
        private string PreencherIdioma(string Campo, int Id)
        {
            if (Request["id"] != null)
            {
                Entity.Tamanho tamanho = new Admin.Tamanho().ConsultarTamanho(Request["id"]);
                switch (Campo)
                {
                    case "Nome":
                        Entity.Dicionario nome = tamanho.Dicionarios.Find(delegate(Entity.Dicionario p1) { return (p1.Idioma_ID == Id && p1.Descricao == "Nome"); });
                        return nome.Valor;
                }
            }
        ...
        /// <summary>
        /// Requisita dados do Tamanho ao banco de dados e preenche o formulário
        /// </summary>
        private void PreencherCampos()
        {
            ...
            // dicionario
            List<Entity.Dicionario> nomes = tamanho.Dicionarios.FindAll(delegate(Entity.Dicionario p1) { return (p1.Descricao == "Nome"); });
            ...
            chkStatus.Checked = tamanho.Status;
        }
        ...
        protected void btnSalvar_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Validar)
            {
                ...
                tamanho.Loja_ID = objUsuario.Loja_ID;
                tamanho.Status = Status;

                // altera registro
                if (Request["id"] != null)
                {
                    tamanho.IDTamanho = Convert.ToInt32(Request["id"]);

                    Entity.Retorno ret = new Admin.Tamanho().AlterarTamanho(tamanho);
                    if (!ret.Status)
                    {
                    ...
                    new Admin.Dicionario().AlterarDicionario(new Entity.Dicionario(objLoja.Idiomas[0].IDIdioma, "Tamanho", tamanho.IDTamanho, "Nome", Nome));
                    for (int i = 1; i < objLoja.Idiomas.Count; i++)
                    {
                        // nome
                        new Admin.Dicionario().AlterarDicionario(new Entity.Dicionario(objLoja.Idiomas[i].IDIdioma, "Tamanho", tamanho.IDTamanho, "Nome", Request.Form["txtNome" + objLoja.Idiomas[i].IDIdioma]));
                    }
                ...
                {
                    Entity.Retorno ret = new Admin.Tamanho().InserirTamanho(tamanho);
                    if (!ret.Status)
                    {
                        // exibe mensagem de erro
                        icon = Icon.remove;
                    ...
                    new Admin.Dicionario().InserirDicionario(new Entity.Dicionario(objLoja.Idiomas[0].IDIdioma, "Tamanho", ret.Identity, "Nome", Nome));
                    for (int i = 1; i < objLoja.Idiomas.Count; i++)
                    {
                        // nome
                        new Admin.Dicionario().InserirDicionario(new Entity.Dicionario(objLoja.Idiomas[i].IDIdioma, "Tamanho", ret.Identity, "Nome", Request.Form["txtNome" + objLoja.Idiomas[i].IDIdioma]));
                    }
